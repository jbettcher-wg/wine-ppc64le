...
/* ppc64le port: Proton can assert here because its unix half is in this
 * process and a failed call means a bug.  Here the unix half is a separate
 * x86-64 Linux process, and "the helper is not there" is the ordinary state
 * of a game launched outside Steam -- the one this bridge must survive
 * legibly.  The assert would abort() the game instead.  steamclient_call()
 * already logs the failure with the call's own name, and every caller's
 * params struct is zero-initialised, so a failed call leaves the SDK's
 * "false"/NULL answer in _ret, which is exactly what a Linux game sees when
 * there is no client. */
