...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, void *reserved)
{
    ...
    switch (reason)
    {
        case DLL_PROCESS_ATTACH:
            ...
            /* ppc64le port: under Proton the ActiveProcess/pid value is
             * written by steam_helper's steam.exe, which owns the prefix's
             * Steam connection.  This port has no steam.exe: the process that
             * owns the connection is the game, and it is this one.  steam_api
             * reads the value to decide whether a client is present, so it is
             * written here, at the moment the module the registry points at
             * is actually loaded. */
            steamrpc_publish_active_process();
            break;
...
void *get_unix_buffer( struct u_buffer buf )
{
    ...
    /* ppc64le port: Proton short-circuits here because its unix half shares
     * this address space, so a u_buffer pointer that fits a UINT_PTR IS a
     * usable pointer.  Here the unix half is a different PROCESS, so buf.ptr
     * is a helper address that always "fits" and never points at anything.
     * The copy below is the only correct path; the short circuit is removed
     * rather than made conditional so it cannot come back by accident.
    if ((UINT_PTR)buf.ptr == buf.ptr && (UINT_PTR)(buf.ptr + buf.len) == (buf.ptr + buf.len))
        return (void *)(UINT_PTR)buf.ptr;
     */
    if (!buf.ptr || !buf.len) return NULL;
