...
| Dex (Windows build) | 269650 | **PE32** | boots, window up, **no D3D11 device** | **the 32-bit lane's first real title** (2026-08-19, `fex nw-dexwin`): the i386 guest boots through the ABI-4 bridge, Unity creates its 314x208 config window — and then dxgi.dll's forwards to `d3d11.__wine_dxvk_*` fail, because the i386 build of d3d11 has NO dxvk thunk exports (`find_forwarded_export` err, four entries).  The named lift: the dxvk thunk surface needs its i386 half.  Everything before D3D11 works, which is exactly what this A/B canary (native i386-Linux Dex on the same box) was picked to show. |
