#!/bin/sh
#
# bench-server.sh -- measure the price of one guest->native crossing on the
...
#
#     WINEPREFIX=... WINEFEXBRIDGE=... ppc64le/cpu/bench-server.sh
#     WINE_PPC64LE_NO_TRAP_VIEW=1 WINEPREFIX=... ppc64le/cpu/bench-server.sh
#
...
#
# The probe is ppc64le/cpu/probes/server_bench.c -- see its header for why
...
skip() { echo "bench-server: $*" >&2; exit 2; }
...
clang -target x86_64-windows-gnu -nostdlib -O2 -fuse-ld=lld \
    ...
    -o "$OUT/server_bench.exe" "$HERE/probes/server_bench.c" "$OUT/libkernel32.a" \
    || skip "guest probe build failed"

echo "bench-server: protocol lines from the launch (view/lazy state):" >&2
"$BUILD/wine" "$OUT/server_bench.exe" 2>"$OUT/stderr.log"
rc=$?
